    ...
    protected function _prepare_basic_auth()
    {
        ...
        if ($username !== null) {
            ...
            if (strpos(strtolower($http_auth), 'basic') === 0) {
				...
				$datalog = explode(':', base64_decode(substr($this->input->server('HTTP_AUTHORIZATION'), 6)));
                if (count($datalog) > 1) {
                    if(filter_var($datalog[0], FILTER_VALIDATE_EMAIL))
                    {
                    ...
                    else
                    {
                        ...
                        $this->response($response, self::HTTP_FORBIDDEN);
                    }
                } else {
                    $this->session->sess_destroy();
                    $response = array(
