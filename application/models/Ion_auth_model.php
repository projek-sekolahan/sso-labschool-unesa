...
class Ion_auth_model extends CI_Model
{
	...
	public function rehash_password_if_needed($hash, $identity, $password)
	{
		...
		if ($algo !== FALSE && $params !== FALSE)
		{
			if (password_needs_rehash($hash, $algo, $params))
			{
				...
				if ($this->_set_password_db($id, $password))
				{
	...
	public function activate($id, $code = FALSE)
	{
		...
		if ($code === FALSE || ($user && $user->id === $id))
		{
			...
			$this->db->update($this->tables['users_login'], $data, ['id' => $id]);
	...
	public function clear_forgotten_password_code($identity) {
		...
		$this->db->update($this->tables['users_login'], $data, ['id' => $identity]);
	...
	public function reset_password($identity, $new) {
		...
		$return	= $this->_set_password_db($id, $new);
	...
	public function username_check($username = '')
	{
		...
		return $this->db->where('username', $username)
						->limit(1)
						->count_all_results($this->tables['users_login']) > 0;
	}
	...
	public function forgotten_password($identity)
	{
		...
		$this->trigger_events('extra_where');
		$this->db->update($this->tables['users_login'], $update, ['id' => $id]);
	 ...
	 * @author    Mathew
	 */
	public function register($identity, $email, $additional_data = [], $groups = [])
	{
		...
		$query = $this->db->get_where($this->tables['groups'], ['id' => $groups['id']], 1)->row();
		...
		$this->db->insert($this->tables['users_login'], $user_data);

		$id = $this->db->insert_id($this->tables['users_login'] . '_id_seq');
	...
	public function login($identity, $password)
	{
		...
		$query = $this->db->select($this->tables['users_details'].'.'.$this->identity_column.','.$this->tables['groups'].'.name as roles,'.$this->tables['users_login'].'.*')
						  ->where($this->tables['users_details'].'.'.$this->identity_column, $identity)
						  ->join($this->tables['users_details'], $this->tables['users_details'].'.'.$this->join['users_details'].'='.$this->tables['users_login'].'.id')
						  ->join($this->tables['users_groups'], $this->tables['users_groups'].'.user_id='.$this->tables['users_login'].'.id')
		...
		if ($query->num_rows() === 1)
		{
			...
			if ($this->verify_password($password, $user->password))
			{
				...
				$this->clear_forgotten_password_code($user->id);
	...
	public function recheck_session()
	{
		...
		if ($recheck !== 0)
		{
			...
			{
				$query = $this->db->select($this->tables['users_login'].'.id')
					->where([
						$this->tables['users_details'].'.'.$this->identity_column => $this->session->userdata('identity'),
						$this->tables['users_login'].'.active' => '1'
					])
					->join($this->tables['users_details'], $this->tables['users_details'].'.'.$this->join['users_details'].'='.$this->tables['users_login'].'.id')
					->limit(1)
					->order_by($this->tables['users_login'].'.id', 'desc')
		...
		return (bool)$session_hash && $session_hash === hash('sha1',base64_encode($this->session->userdata('identity')));
	}
	...
	public function users($groups = NULL)
	{
		...
		if (isset($groups))
		{
			...
			if (isset($groups) && !empty($groups))
			{
				$this->db->distinct();
				$this->db->join(
				    $this->tables['users_groups'],$this->tables['users_groups'].'.'.$this->join['users_details'].'='.$this->tables['users_details'].'.user_id','inner'
				);
			}
			...
			{
				$this->db->join(
					$this->tables['groups'], $this->tables['users_groups'] . '.' . $this->join['groups'] . ' = ' . $this->tables['groups'] . '.id', 'inner'
				);
		...
		$this->db->join(
			$this->tables['users_login'], $this->tables['users_login'] . '.id' . ' = ' . $this->tables['users_details'] . '.user_id', 'inner'
		);
		$this->db->join(
			$this->tables['token'], $this->tables['token'] . '.user_id' . ' = ' . $this->tables['users_details'] . '.user_id', 'inner'
		);
		...
		$this->response = $this->db->get($this->tables['users_details']);
	...
	public function get_users_groups($id = FALSE)
	{
		...
		return $this->db->select($this->tables['users_groups'].'.'.$this->join['groups'].' as id, '.$this->tables['groups'].'.name, '.$this->tables['groups'].'.description')
		                ->where($this->tables['users_groups'].'.'.$this->join['users_login'], $id)
		...
		foreach ($group_ids as $group_id)
		{
			...
			if ($this->db->insert($this->tables['users_groups'],
								  [ $this->join['groups'] => (float)$group_id,
									$this->join['users_login']  => (float)$user_id  ]))
			{
	...
	public function remove_from_group($group_ids = FALSE, $user_id = FALSE)
	{
		...
		if (!empty($group_ids))
		{
			...
			foreach ($group_ids as $group_id)
			{
				...
				$this->db->delete(
					...
					[$this->join['groups'] => (float)$group_id, $this->join['users_login'] => (float)$user_id]
				);
		...
		{
			// Cast to float to support bigint data type
			if ($return = $this->db->delete($this->tables['users_groups'], [$this->join['users_login'] => (float)$user_id]))
			{
	...
	public function update_last_login($id)
	{
		...
		$this->db->update($this->tables['users_login'], ['last_login' => time()], ['id' => $id]);
	...
	public function set_session($user)
	{
		...
		$session_data = [
		    ...
		    'last_check'			=> time(),
			'expired'				=> time()+(7200),
		    'session_hash'			=> hash('sha1',base64_encode($user->email)),
		];
	 ...
	 */
	public function _generate_selector_validator_couple($selector_size, $validator_size)
	{
		...
		if ($user_code)
		{
			...
			if (count($tokens) > 1)
			{
				return (object) [
					'selector'	=> $tokens[0],
					'validator'	=> $tokens[1],
					'mail_code'	=> (empty($tokens[2])) ? $token_code=0:$token_code=$tokens[2],
				];
			}
		}
		return FALSE;
