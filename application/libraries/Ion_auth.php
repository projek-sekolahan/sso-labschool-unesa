...
class Ion_auth
{
	 ...
	 * @author Mathew
	 */
	public function register($identity, $email, $additional_data = [], $group_ids = [])
	{
		$this->ion_auth_model->trigger_events('pre_account_creation');

		$email_activation = $this->config->item('email_activation', 'ion_auth');
		
		$id = $this->ion_auth_model->register($identity, $email, $additional_data, $group_ids);
	 ...
	 * @author Mathew
	 **/
	public function logout($identity)
	{
		...
		// Clear all codes
		$this->ion_auth_model->clear_forgotten_password_code($user->user_id);
	 ...
	 * @param int|string|bool $id
	 *
	 * @return bool Whether the user is an users
	 * @author Ben Edmunds
	 */
	public function is_user($id = FALSE)
	{
		...
		$user_group = $this->config->item('user_group', 'ion_auth');

		return $this->ion_auth_model->in_group($user_group, $id);
	}
